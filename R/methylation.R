...
calculate_MR_tfbs=function(bin_path="tools/PileOMeth/output/MethylDackel",ref_data="",bam="",sample_name="",tf_name="",ref_genome="",tfbs_start=1000,tfbs_end=1000,mapq=10,phred=5,output_dir="",keep_strand=TRUE,bin_width=50,verbose=FALSE,threads=1){
  ...
  tmp_bed(chr=tfbs_to_analyze$chr,start=tfbs_to_analyze$start,end=tfbs_to_analyze$end,strand=tfbs_to_analyze$strand,name=paste0(output_dir,sep,sample_name,"_",tf_name))
	...
	dplyr::mutate(x_bins=as.integer((max(pos_relative_to_tfbs)+min(pos_relative_to_tfbs))/2)) %>%
	dplyr::summarise(MEAN_MR=mean(MR/100,na.rm=TRUE),CI=qt(0.95,(sum(!is.na(MR/100))-1))*sd(MR/100,na.rm=TRUE)/sqrt(sum(!is.na(MR/100))),DATA_POINTS_ANALYZED=sum(!is.na(MR/100)))
	merg_tfbs2= dplyr::rename(merg_tfbs2,POSITION_RELATIVE_TO_TFBS=x_bins) %>% dplyr::mutate(CI95_UPPER_BOUND=ifelse(MEAN_MR+CI>1100,MEAN_MR+CI),CI95_LOWER_BOUND=ifelse(MEAN_MR-CI<0,0,MEAN_MR-CI),TFBS_ANALYZED=nrow(tfbs_to_analyze),BIN_WIDTH=bin_width,TF=paste0(sample_name,"_",tf_name)) %>% dplyr::relocate(TF)
