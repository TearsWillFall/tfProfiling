...
calculate_coverage_around_gp=function(bin_path="tools/samtools/samtools",chr="",position="",strand="",bam="",norm_log2=1,start=1000,end=1000,mean_cov=1,mapq=0,method="default",start_bin=75,end_bin=75){
    if (method=="default"){
      ...
      cov_data_central=read.csv(text=system(paste(bin_path,"view -c -q",mapq, paste0(chr,":",as.numeric(position)-start_bin,"-",as.numeric(position)+end_bin),bam),intern=TRUE),header=FALSE,sep="\t",stringsAsFactors=FALSE)
      ...
      cov_data_left_flank=read.csv(text=system(paste(bin_path,"view -c -q",mapq, paste0(chr,":",as.numeric(position)-start,"-",as.numeric(position)-start_bin),bam),intern=TRUE),header=FALSE,sep="\t",stringsAsFactors=FALSE)
      ...
      cov_data_right_flank=read.csv(text=system(paste(bin_path,"view -c -q",mapq, paste0(chr,":",as.numeric(position)+end_bin,"-",as.numeric(position)+end),bam),intern=TRUE),header=FALSE,sep="\t",stringsAsFactors=FALSE)
